...
# 446 geht nur bis 2022-05-13 11:00:00
# 462 geht nur bis 2022-05-13 11:00:00
# 920 geht nur bis 2022-05-13 11:00:00
...
for(j in min(data$olr_code_id):max(data$olr_code_id)){
  ...
  for(i in (hist.end+1):(nrow(tmp)-4)){
    ...
    a <- tryCatch(
      {
        as.vector(telescope.forecast(hist,h=4,plot = F)$mean)
      },
      error=function(cond) {
        rep(NA,4)
      }
  ...
  write.table(cbind(test,fc),file=paste("results_1h_25p/telescope/telescope_",sprintf("%02d",j),".csv",sep=""),sep=";", row.names = F)
