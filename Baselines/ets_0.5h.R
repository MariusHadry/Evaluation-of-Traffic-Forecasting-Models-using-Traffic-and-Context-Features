...
for(j in min(data$olr_code_id):max(data$olr_code_id)){
  ...
  for(i in (hist.end+1):(nrow(tmp)-8)){
    hist <- ts(tmp[1681:i,2],frequency = 24)
  ...
  write.table(cbind(test,fc),file=paste("results_0.5h_25p/ets/ets_",sprintf("%02d",j),".csv",sep=""),sep=";", row.names = F)
