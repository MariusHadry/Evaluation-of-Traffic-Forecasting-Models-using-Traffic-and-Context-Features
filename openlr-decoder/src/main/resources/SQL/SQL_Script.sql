...
SELECT osm_id, highway, name, lanes, tags, oneway, junction, way FROM planet_osm_line pol WHERE
...
INSERT INTO openlr.kanten (line_id, osm_id, lanes, start_node, end_node, name, fow, frc, oneway, geom)
SELECT fln.id, fln.osm_id, fln.lanes, fln.source, fln.target, fln.name, fln.fow, fln.frc, fln.oneway, st_transform(fln.way, 4326)
