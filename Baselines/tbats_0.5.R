...
for(j in min(data$olr_code_id):max(data$olr_code_id)){
  ...
  for(i in (hist.end+1):(nrow(tmp)-8)){
    hist <- ts(tmp[1681:i,2],frequency = 48*7)
    
    if(is.null(param)){
      model <- tbats(hist, use.parallel=T, num.cores=4)
      param <- model
      
    } else {
      model <- tbats(hist, use.parallel=T, num.cores=4, model=param)
      
    } 
    
    fc <- c(fc, as.vector(forecast(model,h=8)$mean))
  ...
  write.table(cbind(test,fc),file=paste("results_0.5h_25p/tbats/tbats_",sprintf("%02d",j),".csv",sep=""),sep=";", row.names = F)
