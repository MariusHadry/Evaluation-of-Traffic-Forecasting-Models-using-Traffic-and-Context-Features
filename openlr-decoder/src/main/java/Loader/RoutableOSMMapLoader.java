...
import static org.jooq.sources.tables.OpenlrEdges.OPENLR_EDGES;
import static org.jooq.sources.tables.OpenlrNodes.OPENLR_NODES;
 ...
 * Database queries are constructed with the help of JOOQ. This class if for databases
 * with routable data created by the script in the resources' directory.
 *
 * Tables OPENLR_EDGES, OPENLR_NODES, metadata are read here*
 *
 */
...
public class RoutableOSMMapLoader implements MapLoader {
    ...
    public RoutableOSMMapLoader() throws SQLException {
        ...
        System.out.println("-- finished map loading --");
    }
    ...
    public List<NodeImpl> getAllNodes() {
        ...
        List<NodeImpl> allNodes = ctx.select(OPENLR_NODES.NODE_ID, OPENLR_NODES.LAT, OPENLR_NODES.LON)
                .from(OPENLR_NODES).fetchInto(NodeImpl.class);
    ...
    private void setConnectedLinesList(List<NodeImpl> allNodesList) {
        ...
        allNodesList.forEach(n -> {
            List<Long> connectedLinesIDs = ctx.select().from(OPENLR_EDGES)
                    .where(OPENLR_EDGES.START_NODE.eq(n.getID()))
                    .or(OPENLR_EDGES.END_NODE.eq(n.getID()))
                    .fetch().getValues(OPENLR_EDGES.LINE_ID);
    ...
    public List<LineImpl> getAllLines() {
        ...
        List<DirectLine> directLines = ctx.select(OPENLR_EDGES.LINE_ID, OPENLR_EDGES.START_NODE, OPENLR_EDGES.END_NODE, OPENLR_EDGES.FRC, OPENLR_EDGES.FOW,
                        OPENLR_EDGES.LENGTH_METER, OPENLR_EDGES.NAME)
                .from(OPENLR_EDGES)
        ...
        List<ReversedLine> reversedLines = ctx.select(OPENLR_EDGES.LINE_ID, OPENLR_EDGES.START_NODE, OPENLR_EDGES.END_NODE, OPENLR_EDGES.FRC, OPENLR_EDGES.FOW,
                        OPENLR_EDGES.LENGTH_METER, OPENLR_EDGES.NAME)
                .from(OPENLR_EDGES)
                .where(OPENLR_EDGES.ONEWAY.eq(false))
    ...
    private void setLineGeometry(List<LineImpl> linesList) {
        ...
        linesList.forEach(l -> {
            String wktString;
            wktString = ctx.select(st_asText(l.isReversed())).from(OPENLR_EDGES).where(OPENLR_EDGES.LINE_ID.eq(l.getID())).fetchOne().value1().toString();
            try {
