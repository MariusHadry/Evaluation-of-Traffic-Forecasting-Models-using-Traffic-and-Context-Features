...
for(j in min(data$olr_code_id):max(data$olr_code_id)){
  ...
  for(i in (hist.end+1):(nrow(tmp)-4)){
    hist <- ts(tmp[841:i,2],frequency = 7*24)
    ...
    fc <- c(fc, as.vector(forecast(model,h=4)$mean))
  ...
  write.table(cbind(test,fc),file=paste("results_1h_25p/tbats/tbats_",sprintf("%02d",j),".csv",sep=""),sep=";", row.names = F)
