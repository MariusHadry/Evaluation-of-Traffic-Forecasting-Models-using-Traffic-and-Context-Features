    ...
    public int decode(int[] bytes) {
        ...
        if (isValid) {
            ...
            if (!locationReferenceHeader.getIsValid()) {
                ...
                isValid = true;
            } else if (locationReferenceHeader.getGcId() == OLRType.PointAlongLine.id) {
                System.out.println("Point Along Line is not supported!");
                isValid = false;
            } else {
                System.out.println("setting isValid to false. OLR Type: " + locationReferenceHeader.getGcId());
                isValid = false;
            }
        }
        return bytesRead;
